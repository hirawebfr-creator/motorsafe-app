...
const STATUS_CONFIG: Record<string, { bg: string; text: string; label: string; icon: typeof Clock }> = {
  ...
  OVERDUE: { bg: "bg-red-100", text: "text-red-700", label: "En retard", icon: AlertCircle },
...
export default function FacturesPage() {
  ...
  async function load(search: string) {
    try {
      ...
      if (!res.ok) {
        const message = json?.error?.message || json?.error || `GET /api/invoices ${res.status}`;
  ...
  const stats = useMemo(() => {
    ...
    const pendingAmount = items
      ...
      .reduce((acc, i) => acc + (i.totalIncl || 0), 0);
    return { draft, issued, paid, paidAmount, pendingAmount };
  }, [items]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-muted2">Payées</p>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-muted2">CA encaissé</p>
              <p className="text-2xl font-bold">{fmtEur(stats.paidAmount)}</p>
            </div>
      ...
      <Card className="overflow-hidden p-0">
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-muted2">Aucune facture trouvée</p>
            <Link href="/factures/nouveau" className="mt-4 inline-block">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-semibold">
                          {item.invoiceNumber || `Brouillon #${item.id.slice(0, 6)}`}
                        </span>
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-right">
                      ...
                      <p className="text-xs text-muted2">
                        {item.paidAt ? `Payée ${fmtDate(item.paidAt)}` : fmtDate(item.createdAt)}
                      </p>
                    </div>
