...
if (!fs.existsSync(pidPath)) {
	console.log("No .next-dev-fg.pid found (dev server not started via npm run dev).");
	process.exit(0);
}
...
if (stoppedByPort) {
	console.log(`Stopped foreground dev server by port (port=${port}).`);
} else {
	console.log(
		stopped
			? `Stopped foreground dev server (pid=${shownPid}).`
			: `Could not stop pid=${shownPid} (already stopped?).`
	);
}
